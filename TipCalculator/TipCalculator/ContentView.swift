...
struct ContentView: View {
    ...
    @State private var totolAmout : Double = 0
    ...
    var body: some View {
        VStack(spacing: 40){
            ...
            Slider(value: $tipSlider, in: 0...100, step: 1)
                .onChange(of: tipSlider) { _ in
                    ...
                    guard let tip = Calculation().calculateTip(of: amount, with: tipSlider) else{
                        print("Bill amount or tip amount can't be negative")
                        return
                    }
                
                    
                    tipAmount = tip
                    totolAmout = amount + tipAmount
                }
            VStack{
            ...
            VStack{
                Text(totolAmout, format: .currency(code: "USD"))
                    .font(.title.bold())
